...
func parseFlags() (*bool, *string, *string, *string, *string) {
	...
	repoPtr := flag.String("target", "leewannacott/blazingly-fast-github-releases", "Repo where you want to update the releases.")
	changeLogPtr := flag.String("changelog", "docs/CHANGELOG.md", "Relative folder path to changelog.md file")
	help := flag.Bool("help", false, "Example: $ go run main.go --target=leewannacott/blazingly-fast-github-releases --tags=quick-lint/quick-lint-js --changelog=docs/CHANGELOG.md --token=$(cat token.txt)")
	flag.Parse()
	if *authTokenPtr == "" {
